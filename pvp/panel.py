...
# --------- Capture helpers ----------
def _status_bonus(modern_status: Optional[str]) -> float:
...
def _attempt_capture(mon: Mon, ball_name: str, battle_state: BattleState) -> Tuple[bool, int]:
    ...
    # Gen I rough
    if gen == 1:
        f = int(((max_hp * 255 * 4) / (hp * max(1.0, 1.0/ball_mod))) + (10 if getattr(mon, "status", None) in ("slp","frz","sleep","frozen") else 5 if getattr(mon,"status",None) else 0))
        f = max(1, min(255, f))
        ...
        for _ in range(3):
            if random.randint(0, 255) < f:
                ...
                break
        return False, shakes
    # Gen II separate but we approximate with modern formula
    # Modern formula Gen III+
    ...
    if gen >= 8:
        if "raid" in (battle_state.fmt_label or "").lower():
            difficulty = 2.0
            bonus_misc *= difficulty
    a = ((3 * max_hp - 2 * hp) * base_rate * ball_mod * status_mod * bonus_level * bonus_misc) / (3 * max_hp)
    # Gen V+ uses *4096 internally
    if gen >= 5:
        a = a * 4096
        # Critical capture chance (Gen V+): approx sqrt species caught; simplified: 0 for now
        # Success threshold
        if a >= 1044480:  # auto-catch threshold Gen V+
            return True, 4
        b = int(65536 * pow(a / 1044480.0, 0.1875 if gen >= 6 else 0.25))
        # Critical capture uses fewer shakes (1); if it fails, fall back to normal sequence
        if crit_capture:

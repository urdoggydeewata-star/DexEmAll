"""
Ball and friendship mechanics - extracted from pokebot.py.
Pure helpers for Poké Ball identification and friendship-on-catch behavior.
"""
...
def caught_hp_for_ball(mon: Any, hp_max: int, ball_item_id: Optional[str]) -> int:
    """Apply on-catch HP behavior from the catch ball."""
    if is_heal_ball(ball_item_id):
